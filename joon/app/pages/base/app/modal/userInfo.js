...
export default class userInfo extends Component {
  ...
  footer() {
    return (
      <div>
        <div className="gout-btn"><Button type="primary" onClick={this.props.handleLogout}>로그아웃</Button></div>
      </div>
    )
  }
  render() {
      ...
      >
        <div className="user">
          ...
          <div className="user-info">
            <ul>
              <li><span>이름</span><b>{userinfo.chineseName}</b></li>
              <li><span>연락처</span><b>{userinfo.phoneNo}</b></li>
              <li><span>연락처2</span><b>{userinfo.shortPhoneNo}</b></li>
              <li><span>위치정보</span><b>{userinfo.deptName}</b></li>
              <li><span>직책</span><b>{userinfo.post}</b></li>
              <li><span>권한</span><b>{roles}</b></li>
              <li className="changePsw_in"><span>비밀번호변경</span><i className="enter" onClick={() => this.setState({ pswFlag: true })}>변경</i></li>
              {this.state.pswFlag ?
                <div className="changePswWrap">
                  <div className="changePsw">
                    <div className="changePsw_title">비밀번호변경</div>
                    <Form layout="horizontal" onSubmit={this.handleSubmit}>
                      <FormItem {...formItemLayout} label="기존 비밀번호" hasFeedback>
                        {getFieldDecorator('oldPass', {
                          initialValue: '',
                          rules: [{ required: true, message: '기존 비밀번호를 입력하세요' },
                            { pattern: regExpConfig.pwd, message: '비밀번호는 6-16글자입니다' },
                          ],
                        })(<Input placeholder="기존 비밀번호를 입력하세요" type="password" />)}
                      </FormItem>
                      <FormItem {...formItemLayout} label="새로운비밀번호" hasFeedback>
                        {getFieldDecorator('password', {
                          initialValue: '',
                          rules: [{ required: true, message: '새로운비밀번호를 입력하세요' },
                            { pattern: regExpConfig.pwd, message: '비밀번호는 6-16글자입니다' },
                            { validator: this.checkRepeatPassword },
                          ],
                        })(<Input placeholder="새로운비밀번호를 입력하세요" type="password" />)}
                      </FormItem>
                      <FormItem {...formItemLayout} label="새로운비밀번호 확인" hasFeedback>
